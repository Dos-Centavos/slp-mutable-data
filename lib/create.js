...
class Create {
  ...
  async createToken (wif, tokenData, mda, destAddr, tokenType = 'fungible') {
    try {
      ...
      // Build Token
      const hex = await this.buildTokenTx(wif, slpData, destAddr, tokenType)
  ...
  async buildTokenTx (wif, slpData, destAddr, tokenType = 'fungible') {
    try {
      ...
      if (tokenType === 'group') {
        ...
        script = this.bchjs.SLP.NFT1.generateNFTChildGenesisOpReturn(slpData)
      } else {
