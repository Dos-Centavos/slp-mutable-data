...
class Create {
  constructor (localConfig = {}) {
    ...
    this.buildTokenTx = this.buildTokenTx.bind(this)
    this.buildNftTokenTx = this.buildNftTokenTx.bind(this)
    this.createMutableTxid = this.createMutableTxid.bind(this)
  ...
  async createToken (wif, tokenData, mda, destAddr, tokenType = 'fungible', groupId) {
    try {
      ...
      const txid = await this.retryQueue.addToQueue(this.wallet.broadcast, { hex })
  ...
  async createMutableTxid (wif, mda) {
    try {
      ...
      // const txidStr = await this.wallet.ar.sendTx(hex)
      const txidStr = await this.retryQueue.addToQueue(this.wallet.broadcast, { hex })
  ...
  buildMintTx (tokenId, destAddr) {
    try {
      ...
      // console.log(`mintBatons: ${JSON.stringify(mintBatons, null, 2)}`)
      ...
      // Destroy the baton?
      const destroyBaton = false
      // if (!destAddr) destroyBaton = true

      // Generate the OP_RETURN entry for an SLP MINT transaction.
      let script
      const qty = 1
      if (mintBaton.tokenType === 129) {
        script = this.bchjs.SLP.NFT1.mintNFTGroupOpReturn([mintBaton], qty, destroyBaton)
      } else {
        // tokenType === 1 (fungible token)
        script = this.bchjs.SLP.TokenType1.generateMintOpReturn([mintBaton], qty, destroyBaton)
      }
      ...
      if (!destroyBaton) {
        if (destAddr) {
          transactionBuilder.addOutput(
            this.bchjs.Address.toLegacyAddress(destAddr),
            546
          )
        } else {
          transactionBuilder.addOutput(
      ...
      console.error('Error in buildMintTx()')
      throw err
    }
  }

  async mintToken (groupId, destAddr) {
    try {
      const hex = await this.buildMintTx(groupId, destAddr)
      // const txid = await this.bchjs.RawTransactions.sendRawTransaction([hex])
      // const txid = await this.wallet.ar.sendTx(hex)
      const txid = await this.retryQueue.addToQueue(this.wallet.broadcast, { hex })
      ...
      console.error('Error in mintToken()')
      throw err
    }
  }
}
