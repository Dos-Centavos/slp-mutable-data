...
class Create {
  ...
  async createToken (wif, tokenData, mspAddr, destAddr) {
    try {
      ...
      // Build Token
      const hex = await this.buildTokenTx(wif, slpData, destAddr)
  ...
  async buildTokenTx (wif, slpData, destAddr) {
    try {
      ...
      if (Array.isArray(utxos)) {
        bchUtxos = utxos[0].bchUtxos
      } else {
        bchUtxos = utxos.bchUtxos
      }
      const utxo = await this.bchjs.Utxo.findBiggestUtxo(bchUtxos)
  ...
  async createMutableTxid (wif, mspAddr) {
    try {
      ...
      if (!mspAddr || typeof mspAddr !== 'string') {
        throw new Error('mspAddr must be a string')
      }
