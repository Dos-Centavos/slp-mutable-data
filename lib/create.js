...
class Create {
  ...
  async createMutableTxid (wif, mda) {
    try {
      ...
      let txidStr = await this.wallet.ar.sendTx(hex)

      // TODO: When using rest-api interface, the TXID is an array. Rather than
      // handling this here, the root cause in minimal-slp-wallet should be
      // changed to ensure it's giving strings and not arrays.
      if (Array.isArray(txidStr)) txidStr = txidStr[0]
