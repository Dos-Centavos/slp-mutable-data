...
class Data {
  ...
  // Initialize this class by passing handles to bchjs and the web3 library.
  // This is used so these things can loaded after the class is instantiated.
  initialize (localConfig = {}) {
    try {
      ...
      console.log('Error in data.js/initialize()')
      throw err
    }
  }
  ...
  async uploadToFilecoin (data, apiKey) {
    try {
      if (!this.isInitialized) {
        throw new Error('Call initialize() before calling this function.')
      }
  ...
  makeFileObjects (obj) {
    ...
    const files = [new this.File([buffer], 'mutable-data.json')]
    return files
  }
